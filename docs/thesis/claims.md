...
| N-16 | ReLU on (as in the donor network) | **mechanism confirmed, deliverable marginal, and IT SHIPS** — refuted on the 3x3/16 mobilenet bank, wins four times offline on a LEARNED Layer-1 bank and LOSES on an analytic Gabor one, so the property that matters is that the bank is learned. **dEAO +0.0029 against a pre-registered +0.005 bar: NOT accepted on its own falsifier**, and it ships on the paired R result instead — never write it up as a pass. **The linear twin `ARM=l1lin` has NOT run**, so the gain belongs to the arm, not yet to the rectifier | `evidence/arm_l1relu.md` sec.10-12, `evidence/layer1_features.md`; numbers in `results/arms.csv` row `rgb_l1relu` | `subsec:wyborSieci` |
