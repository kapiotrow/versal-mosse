...
| R-11 | The MAINLOBE WIDTH `sigma/target` is the axis, with an optimum at 1/16 (DSST's `target/16`) | **confirmed on hardware 2026-09-01, and it is the BEST ARM ON RECORD**: `MOSSE_SIGMA=4.0` at 128x128 gives A 0.5133 / R 0.4095 / EAO **0.1931**, dEAO +0.0301 (bar +0.005), drop-top-3 +0.0235, accuracy +0.0299 on the common survived prefix. HOST-ONLY. At matched sigma/target the finer map beats the coarser one (+0.0222 R), the OPPOSITE of the offline proxy | `evidence/proposed_build_res64.md` sec.21, sec.25 | `sec:jakoscSledzenia`, `sec:dyskusjaWynikow` |
...
| N-16 | ReLU on (as in the donor network) | **BANK-SPECIFIC, and the scope is now measured.** On the SHIPPING 3x3/16 bank it is refuted (pooled dR relu −0.0332, abs −0.0232, crelu +0.0020). On a LAYER-1 bank it beats its own linear twin — **+0.0398** (vgg16 conv1 3x3 + maxpool) and **+0.0599** (resnet18 7x7/2), the first time a nonlinearity has beaten a matched linear control here. But no Layer-1 arm beats the geometry-matched control after drop-top-3, and every arm ran at the 64x64 map that hardware measured as the WORSE geometry | `evidence/layer1_features.md`, `evidence/feature_bank.md` | `subsec:wyborSieci` |
...
| O-04 | Feature-bank **geometry** (16ch / 3×3 / stride 1) — not the weights | not refuted; an AIE rebuild. **As of 2026-09-01 it is the ONLY feature axis left**: the weights are a linear lift (one-hot ties the network) and the pointwise nonlinearity is refuted on AR (N-16) | `evidence/feature_bank.md` |
